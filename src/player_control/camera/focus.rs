...
pub(super) fn set_camera_focus(
    ...
    //     let dialog_target_transform =
    // dialog_targets.get(dialog_target).unwrap();     camera.
    // secondary_target = Some(dialog_target_transform.translation); }
    camera.target =
        player_transform.translation + Vec3::Y * player::HEIGHT / 2.;
